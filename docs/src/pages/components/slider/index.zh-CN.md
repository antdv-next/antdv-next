...
| disabled | 值为 true 时，滑块为禁用状态；在 Range 模式下可传 `boolean[]` 单独禁用某个滑块 | boolean \| boolean[] | false | boolean[]: 1.4.0 | × |
...
| tooltip | 设置 Tooltip 相关属性 | [TooltipConfig](#tooltipconfig) | - | - | × |
...
| editable | 启动动态增减节点，不能和 `draggableTrack` 一同使用 | boolean | false | - |
| minCount | 配置 `editable` 时，最小节点数量 | number | 0 | - |
| maxCount | 配置 `editable` 时，最大节点数量 | number | - | - |
...
| formatter | Slider 会把当前值传给 `formatter`，并在 Tooltip 中显示 `formatter` 的返回值，若为 null，则隐藏 Tooltip | (value: number) =&gt; VueNode \| null | IDENTITY | - |
| getPopupContainer | Tooltip 渲染父节点，默认渲染到 body 上 | (triggerNode: HTMLElement) =&gt; HTMLElement | () =&gt; document.body | - |
