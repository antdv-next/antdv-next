...
如果你开启了`nuxt devtools`，会拖慢整个样式在家的速度，如果您遇到了在开发模式下加载样式过慢或者无法正常点击的情况，请尝试关闭`nuxt devtools`再试试，或者等待`console`中已经完成加载后再进行操作。

正常开发环境由于是预编译不会出现这个问题，生产环境也不会出现这个问题，所以不必担心。
